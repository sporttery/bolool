...
public class CalcAllHigh implements ICalc {

	private BigDecimal AOBO_WIN_RACE = null;
	private BigDecimal DANCHANG_FANDIAN = null;

	public CalcAllHigh() {
		AOBO_WIN_RACE = ICalc.AOBO_WIN_RACE;
		DANCHANG_FANDIAN = ICalc.DANCHANG_FANDIAN;
	}

	public CalcAllHigh(BigDecimal aoBoWinRace, BigDecimal danchangFanDian) {
		AOBO_WIN_RACE = aoBoWinRace;
		DANCHANG_FANDIAN = danchangFanDian;
	}

	public CalcAllHigh(BigDecimal aoBoWinRace) {
		AOBO_WIN_RACE = aoBoWinRace;
		DANCHANG_FANDIAN = ICalc.DANCHANG_FANDIAN;
	}

	private String remark = "北单双选，澳博全高赔";
	...
	public void calcByIssue(String issue, String status) {
		...
		try {
			...
			for (; i < maxLen; i++) {
				...
				BigDecimal maxWDL1 = match.getMaxWDL1();
				boolean aoBoWin = maxWDL1 == match.getW1() && "W".equals(aoBoResult)
						|| maxWDL1 == match.getD1() && "D".equals(aoBoResult)
						|| maxWDL1 == match.getL1() && "L".equals(aoBoResult);
				...
				aoBoWinBet = totalBetMoney.multiply(AOBO_WIN_RACE).divide(maxWDL1.subtract(SP_VALUE), 0,

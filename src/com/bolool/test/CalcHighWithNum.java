...
public class CalcHighWithNum implements ICalc {
	private String join() {
		return join(highNum);
	}

	private String join(Object[] highNum) {
		String str = "";
		for (int i = 0; i < highNum.length - 1; i++) {
			str += highNum[i] + ",";
		}
		str += highNum[highNum.length - 1];
		return str;
	}
	...
	public CalcHighWithNum(Integer[] highNum) {
		this.highNum = highNum;
		this.remark = "澳博指定场次高赔" + join();
	}
	...
	public void calcByIssue(String issue, String status) {
		...
		try {
			...
			List<Integer> highNumList = Arrays.asList(highNum);
			for (; i < maxLen; i++) {
				TDanchangMatch match = rows.get(i);
				String result = match.getResult();
				String aoBoResult = match.getAoBoResult();
				Integer num = new Integer(i + 1);
				boolean aoBoWin = false;
				if (highNumList.contains(num)) {
					aoBoWinSp = match.getMaxWDL1();
				} else {
				...
				aoBoWin = aoBoWinSp == match.getW1() && "W".equals(aoBoResult)

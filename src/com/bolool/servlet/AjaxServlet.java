...
//		System.out.println(boloolJSON);
		...
		map.forEach((key,value)->{
			...
			//matchId,hscore,ascore,hresult,aresult,hsection,asection,hstrong,astrong,topN,friendly
		});
		for (String key : boloolData.keySet()) {
			Bolool bolool = boloolData.get(key);
			boloolCount += DBHelper.insertOrUpdatePre(Const.BOLOOL_SQL,bolool.getMatchId(),bolool.getHscore(),bolool.getAscore(),bolool.getHresult(),bolool.getAresult(),bolool.getHsection(),bolool.getAsection(),bolool.getHstrong(),bolool.getAstrong(),bolool.getTopN(),bolool.getFriendly() );
		}
//		System.out.println(boloolData.get("top33").getAresult());
		
		
		int matchCount = DBHelper.insertOrUpdatePre(Const.MATCH_SQL,match.getId(),match.getLeagueId(),match.getLeagueName(),match.getLeagueType(),match.getSeasonId(),match.getSeasonName(),match.getRound(),match.getHomeId(),match.getHomeName(),match.getAwayId(),
				match.getAwayName(),match.getPlaytime(),match.getFullscore(),match.getHalfscore(),match.getResult(),match.getGoalscore());
		...
		response.getWriter().write("{\"matchCount\":"+matchCount+",\"historyCount\":"+historyCount+",\"boloolCount\":"+boloolCount+"}");
	...
	private void getBoloolList(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		String column = "id,leagueName,leagueId,seasonName,round,playtime,homeName,homeId,awayId,awayName,fullscore,halfscore,s,p,f,h,pan,a";
		String sql = "select m."+column+" from t_match m left join t_match_odds o on m.id = o.matchId  ";
...
//		String sql = "select m."+column+" from t_match m left join t_match_odds o on m.id = o.matchId left join t_match_history h on m.id = h.id ";
		if(europeArr.length==3 && asiaArr.length == 3) {
			String sql1 = sql + " where o.s = " + europeArr[0] + " and o.p = " + europeArr[1] + " and o.f = " + europeArr[2] ;
			String sql2 =  sql + " where o.h = "+ asiaArr[0] + " and o.pan = '"+asiaArr[1]+"' and o.a="+asiaArr[2];
	...
	private void getMatchHistory(HttpServletRequest request, HttpServletResponse response)throws IOException  {
		...
		if(StringUtils.isEmpty(ids) ) {
			...
			String sql = "select m."+column+" from t_match m left join t_match_history h on m.id = h.id where m.id in ("+ids+")";
	...
	private String getFromOkooo(String url, String method, String args) {
		...
		if ("get".equals(method)) {
			if(url.indexOf("history")!=-1) {
				...
				String file = getServletContext().getRealPath("/curlMatchHistory.sh");
				return HttpUtil.execCurl(new String[] {"/bin/sh",file,id});
			}

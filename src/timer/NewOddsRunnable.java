...
public class NewOddsRunnable implements Runnable {
	...
	private void getOddsByIds(String ids, int saveOrUpdate) {
		...
		if (odds != null) {
			...
			try {
				map = new Gson().fromJson(odds, type);
			} catch (Exception e) {
			...
			if (map != null) {
				map.forEach((key, value) -> {
					...
					// int result = DBHelper.insertOrUpdatePre(Const.MATCH_ODDS_EUROPE_UPDATE,
					// Float.valueOf(value[0]),
			...
			if (saveOrUpdate == UPDATE) {
				oddsResult.forEach((key, value) -> {
					int result = DBHelper.insertOrUpdatePre(Const.MATCH_ODDS_UPDATE, Float.valueOf(value[0]),
							Float.valueOf(value[1]), Float.valueOf(value[2]), Float.valueOf(value[3]), value[4],
							Float.valueOf(value[5]), Integer.valueOf(key));
					log.info(" matchId:" + key + " 更新ODDS入库 " + result);
				});
			} else {
				oddsResult.forEach((key, value) -> {
					int result = DBHelper.insertOrUpdatePre(Const.MATCH_ODDS_INSERT, Integer.valueOf(key),
							Float.valueOf(value[0]), Float.valueOf(value[1]), Float.valueOf(value[2]),
							Float.valueOf(value[3]), value[4], Float.valueOf(value[5]));
					log.info(" matchId:" + key + " 插入ODDS入库 " + result);
				});
			}
		}
	}
